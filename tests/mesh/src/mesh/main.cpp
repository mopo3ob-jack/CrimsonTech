...
int main() {
	...
	if (Mesh::convert("./resources/models/room.fbx", "./mesh.ctm", arena)) {
		return 1;
	}

	VertexArray mesh;
	GLsizei elementCount = Mesh::load("./mesh.ctm", mesh, arena);
	if (elementCount == 0) {
		return 1;
	}
